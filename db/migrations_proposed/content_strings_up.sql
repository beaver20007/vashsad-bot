...
--
-- НЕ УЛАЖЕНО (называю явно, не молчу): значения для namespace
-- 'order_status' — см. блок ниже перед местом, где был бы INSERT.
...
-- order_status.* — СОЗНАТЕЛЬНО НЕ ЗАСЕЯНО здесь.
--
-- Причина: бот и miniapp расходятся не только именем таблицы (уже
-- сведено выше), но и содержанием этого namespace. Бот несёт свою
-- продовую формулировку (ORDER_STATUS_INFO в
-- handlers/admin_bot_handlers.py: admin_label + client_text для каждого
...
-- выше. До этого явного решения namespace 'order_status' здесь не
-- трогаю, чтобы не создать скрытую гонку миграций.
--
-- Когда решение будет: добавить сюда INSERT с ON CONFLICT DO UPDATE
-- (как designer_bio выше) и синхронно поправить/убрать конфликтующие
-- строки в miniapp's 020 — координировать оба репозитория одним PR-окном.
